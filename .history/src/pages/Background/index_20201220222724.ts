...
import { getAllDataFromStorage } from '../../helpers';
...
chrome.commands.onCommand.addListener(async (command) => {
  ...
  const data = await getAllDataFromStorage();
  ...
  switch (command) {
    case DECREASE_PLAYBACK_RATE:
      chrome.tabs.query(targetTab, (tabs) => {
        ...
        // send to current tab
        if (tabs[0].id) {
          console.log('send message to content from background', data);
          chrome.tabs.sendMessage(tabs[0].id, {
            type: SET_PLAYBACK_RATE,
            payload: { targetRate: undefined },
          });
        }
      });
      break;
